...
def anime_data(raw_data:str):
    ...
    verified_titles = (
        raw_info_tab.find_all("tr", {"class": "official verified yes"}) +
    ...
    unverified_titles = (
        raw_info_tab.find_all("tr", {"class": "official verified no"}) +
    ...
    raw_staff = sauce.find("div",
                          {"class": "g_section g_bubble staff"})
    staff = {}
    for category in raw_staff.find_all("tr", {"class": "g_odd"}):
        ...
        for i in item[1]:
            ...
            try:
                author = data_types.Url(name = i.text,
                                        link = SITE_URL+i['href'])
            except:
                pass
            else:
